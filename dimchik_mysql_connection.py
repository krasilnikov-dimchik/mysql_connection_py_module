...
class DimchikMysqlConnection:
    ...
    def connection(self):
        ...
        try:
            ...
            self.connect = mysql.connector.connect(user=self.user, host="localhost",
    ...
    def use_database(self, database_name):
        ...
        self.__conn.cmd_init_db(database_name)
        # Trying to use the current database
        print("Used database:", self.__conn.database)

    def create_table(self, table_name, query):
        # Create a table
        print(f"Creating the table {table_name} ...")
        self.__cursor.execute(f"CREATE TABLE {table_name} {query}")
    ...
    def insert_to_table(self, table_name, columns, query):
        ...
        self.__cursor.execute(f"INSERT INTO {table_name} {columns} VALUES {query}")
    ...
    def describe_table(self, database_name, table_name):
        ...
        self.__cursor.execute(f"DESCRIBE {table_name}")
    ...
    def show_table_in_database(self, database_name, table_name=''):
        ...
        if not table_name:
            for self.table in self.rows:
                ...
                self.__cursor.execute(f"SELECT * FROM {self.table[0]}")
                self.items = self.__cursor.fetchall()
    ...
    def update(self, table_name, set_condition, where_condition):
        ...
        self.__cursor.execute(f"UPDATE {table_name} SET {set_condition} WHERE {where_condition}")
    ...
    def join_tables(self, table_left_name, table_right_name, join_type, condition_left, condition_right):
        ...
        self.__cursor.execute(f"SELECT * FROM {table_left_name} {join_type} {table_right_name} "
                              f"on {condition_left} = {condition_right} ")
        self.rows = self.__cursor.fetchall()
        self.print_func(self.rows)
    ...
    def is_table_exist(self, database_name, table_name):
        ...
        self.__cursor.execute(f"USE {database_name}")
        self.__cursor.execute(f"SHOW TABLES")
        self.tables = self.__cursor.fetchall()
        self.res = False
    ...
    def is_database_exist(self, database_name):
        ...
        self.__cursor.execute(f"SHOW DATABASES")
        self.databases = self.__cursor.fetchall()
        self.res = False
